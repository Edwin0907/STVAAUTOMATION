...
public class AppNewFunctions extends Globals{
	...
	public boolean Fn_Com_Navigate(String strPage)		//HOme_OnDemand
	{
	...
	{
		EF.Update_Step_Report("User must be navigated to Kid zone screen","User Navigated to Kid zone Screen||User not navigated to Kidzone Screen",ST.NativeElementFound(ObjectCollection.get("KidZone_Banner").toString(), client));
		client.sleep(10000);
	...
	public void Fn_Kidzone_ParentalPopOver()
	{
		...
		EF.Update_Step_Report("Parental control pop over should display","Parental control pop Over displayed||Parental control Pop Over is not displayed", ST.NativeElementFound(ObjectCollection.get("Home_Page_PopOver").toString(), client));
		ST.NativeClick(ObjectCollection.get("Home_btn_PopOver").toString(), client);
			...
			{	
				...
				if(ST.NativeElementFound(ObjectCollection.get("Login_Frame_EnvPopUp").toString(), client))
				{
					...
					else
					{
						...
						client.sleep(3000);
						if(client.elementSwipeWhileNotFound("NATIVE", "xpath=//*[@class='UIScrollView']", "Down", 0, 500, "NATIVE", "xpath=//*[@accessibilityLabel='PROD']", 0, 1000, 5, true))
						{
	...
	public boolean Fn_LogOut()
	{
		...
		try 
		{
			if(this.Fn_Com_Navigate("Home_Settings"))
			{
	...
	public boolean Fn_SportZone_InHome()
	{
		...
		if(this.Fn_Login())
		{
			...
			//					ST.NativeClick(ObjectCollection.get("BackButton").toString(), client);
			//					//client.click("NATIVE", ObjectCollection.get("BackButton").toString(), 0, 1);
			//					EF.Update_TC_Report();
			//					//Auto_SportZone_010
			//					this.Rest2homescreen();
			//					EF.Update_Step_Report("The App must be downloaded and installed in the device", "The App is downloaded and installed||The App is not downloaded and installed", true);
			...
			client.swipeWhileNotFound("Up", 500, 500, "NATIVE", ObjectCollection.get("SportZone_Today").toString(), 0, 1000, 5, false);
			EF.Update_TC_Report();
			//Auto_SportZone_013
			EF.Update_Step_Report("The App must be downloaded and installed in the device", "The App is downloaded and installed||The App is not downloaded and installed", true);
			EF.Update_Step_Report("User should eb able to Login Successfully", "User is logged in Successfully||User is not Logged in Successfully", true);
			...
			EF.Update_Step_Report("Filter options should be displayed in the SportZone page", "Filter option is displayed||Filter option is not displayed", ST.NativeElementFound(ObjectCollection.get("SportZone_filterOption_ALL").toString(), client));
			EF.Update_TC_Report();
			// Auto_SportZone_052
			EF.Update_Step_Report("The App must be downloaded and installed in the device", "The App is downloaded and installed||The App is not downloaded and installed", true);
			EF.Update_Step_Report("User should eb able to Login Successfully", "User is logged in Successfully||User is not Logged in Successfully", true);
...
//	  		System.out.println("AvailableKeys Count: "+ availableKeys.size());
	...
	public void Auto_IPVOD_Validations()
	{
		...
		{
			ST.NativeClick(ObjectCollection.get("Login_PopUp_Okbtn").toString(), client);
			EF.Update_Step_Report("Check for Streaming the asset from Last Channels under Last Channels Shelf", "The asset from Last Channels under Last Channels Shelf is not getting streamed||An error message occured on streaming last channels", false);
		}
		else
		{
			if(ST.NativeElementFound(ObjectCollection.get("VideoPlayer_Btn_Done").toString(), client))
			{
				EF.Update_Step_Report("Check for Streaming the asset from Last Channels under Last Channels Shelf", "The asset from Last Channels under Last Channels Shelf is getting streamed||Streaming is not happening the asset from Last Channels under Last Channels Shelf",
		...
		{
			if (Temp_Var.equalsIgnoreCase("Browse On Demand to add Movies and TV Shows."))
				Local_Result = true;
			else
				Local_Result = false;
		}
		...
		EF.Update_Step_Report("Browse All link should be hidden when Downloads Shelf with Zero downloads", "Browse All link should not be hidden when the Downloads Shelf with Zero downloads||Browse All link should be hidden when the Downloads Shelf with Zero downloads", ST.NativeElementFound(ObjectCollection.get("Home_Downloads_BrowseAll_hidden").toString(), client));
		...
		EF.Update_Step_Report("Display of Edit button under My Favorites shelf", "Edit button should be available under My Favorites Shelf||Edit button is not available under My Favorites Shelf",
	...
	public void Fn_Showtime_DownloadIcon()
	{
		...
		if(str.equalsIgnoreCase("showtime"))
		{
			EF.Update_Step_Report("Should display the networks available for the account", "Contents are displayed||Contents are not displayed",true);
			client.waitForElement("NATIVE",ObjectCollection.get("BrowseAll").toString(), 0, 30000);
			EF.Update_Step_Report("Should display the assets provided by Showtime Network", "Contents are displayed||Contents are not displayed",
					ST.NativeElementFound(ObjectCollection.get("BrowseAll").toString(), client));
			ST.NativeClick(ObjectCollection.get("BrowseAll").toString(), client);
			if(Current_OS.equalsIgnoreCase("Android"))
				ST.NativeClick(ObjectCollection.get("Asset_CoverArt").toString(), client);
			client.sleep(7000);
			EF.Update_Step_Report("Asset detail page should be displayed with the asset title and description", "The asset title and description is displayed||The asset title and description is missing", ST.NativeElementFound(ObjectCollection.get("AssetDetails_ProgramTitle").toString(), client));
//			EF.Update_Step_Report("Asset detail page should be displayed with the ratings", "Asset details page is displayed with ratings||Asset details page is not displayed with ratings", ST.NativeElementFound(ObjectCollection.get("RatingsOverview_TVRatings").toString(), client)||ST.NativeElementFound(ObjectCollection.get("RatingsOverview_MovieRatings").toString(), client));
			EF.Update_Step_Report("Asset detail page should be displayed with the D2G icon and size of selected asset", "Asset details page is displayed with D2G icon and Size||Asset details page is not displayed with D2G icon and Size", ST.NativeElementFound(ObjectCollection.get("AssetDetailsScreen_DownloadIcon").toString(), client));
...
//	  		System.out.println("AvailableKeys Count: "+ availableKeys.size());
	...
	{
		try{
		int xcurrentTime = Integer.parseInt(client.elementGetProperty("NATIVE", "xpath=//*[@contentDescription='Now indicator for the guide.']", 0, "x"));
		int xTimeCell = Integer.parseInt(client.elementGetProperty("NATIVE", "xpath=//*[@class='com.charter.tv.guide.TimeCell' and @x<" + xcurrentTime + " and " +
				"@x+@width > " + xcurrentTime + "]", 0, "x"));
		int widthTimeCell = Integer.parseInt(client.elementGetProperty("NATIVE", "xpath=//*[@class='com.charter.tv.guide.TimeCell' and @x<" + xcurrentTime + " and " +
				"@x+@width > " + xcurrentTime + "]", 0, "width"));
		String time = client.elementGetText("NATIVE", "xpath=//*[@class='com.charter.tv.guide.TimeCell' and @x<" + xcurrentTime + " and " +
				"@x+@width > " + xcurrentTime + "]//*[@id='guide_time_cell_text']", 0);
		...
		int timeFrame = 30;
		float addition = (float)xcurrentTime/((float)xTimeCell + (float)widthTimeCell)*(float)timeFrame;
		System.out.println(addition);
		cal.add(Calendar.MINUTE, (int)addition);
